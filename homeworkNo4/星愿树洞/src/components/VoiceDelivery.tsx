...
import { playCelestialChime, playCosmicSwell } from '../lib/sound';
...
export function VoiceDelivery({ onWishDelivered }: VoiceDeliveryProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    playCosmicSwell(); // play atmospheric cosmic swell on submit start
    ...
    try {
      ...
      setTimeout(() => {
        ...
        playCelestialChime(); // play bright resonant chime when delivered successfully!
      }, 2500);
  ...
  return (
    <div className="w-full max-w-4xl mx-auto space-y-10 pt-6 px-4">
        ...
        {!lastOrionReply ? (
          ...
          >
            ...
            >
              ...
              {submissionProgress && (
                <div className="absolute inset-0 z-20 bg-black/95 flex flex-col items-center justify-center p-6">
                  <CelestialLoading 
                    label="DELIVERING HEART STREAM TO GALAXY..." 
                    sublabel="信信正转化为星光，随银河波谱飞往守护者 Orion。正在倾听你的呼吸声..." 
                  />
                </div>
              )}
      ...
      {/* Breathing meditation companion widget */}
      <BreathingGuide />
